...
( function() {'use strict';
		...
		THREE.TransformGizmo = function() {
			...
			var scope = this;
			var showPickers = false;
			//debug
			var showActivePlane = false;
			//debug
		...
		THREE.TransformGizmoTranslate = function() {
			...
			this.handleGizmos = {
				...
				XY : [[new THREE.Mesh(new THREE.PlaneGeometry(0.29, 0.29), new GizmoMaterial({
					color : 0xffff00,
					opacity : 0.25
				})), [0.15, 0.15, 0]]],
				YZ : [[new THREE.Mesh(new THREE.PlaneGeometry(0.29, 0.29), new GizmoMaterial({
					color : 0x00ffff,
					opacity : 0.25
				})), [0, 0.15, 0.15], [0, Math.PI / 2, 0]]],
				XZ : [[new THREE.Mesh(new THREE.PlaneGeometry(0.29, 0.29), new GizmoMaterial({
					color : 0xff00ff,
					opacity : 0.25
			...
			this.pickerGizmos = {
				...
				XY : [[new THREE.Mesh(new THREE.PlaneGeometry(0.4, 0.4), new GizmoMaterial({
					color : 0xffff00,
					opacity : 0.25
				})), [0.2, 0.2, 0]]],
				YZ : [[new THREE.Mesh(new THREE.PlaneGeometry(0.4, 0.4), new GizmoMaterial({
					color : 0x00ffff,
					opacity : 0.25
				})), [0, 0.2, 0.2], [0, Math.PI / 2, 0]]],
				XZ : [[new THREE.Mesh(new THREE.PlaneGeometry(0.4, 0.4), new GizmoMaterial({
					color : 0xff00ff,
					opacity : 0.25
				...
				if (_mode == "translate") {
					...
					if (scope.space == "local") {
						...
						if (scope.axis == "XYZ") {
							...
							if (scope.axis == "X")
								scope.object.scale.x = oldScale.x * (1 + point.x / (50 / (20 * distance)));
							if (scope.axis == "Y")
								scope.object.scale.y = oldScale.y * (1 + point.y / (50 / (20 * distance)));
							if (scope.axis == "Z")
								scope.object.scale.z = oldScale.z * (1 + point.z / (50 / (20 * distance)));
